#!/usr/bin/env python3
"""
Hybrid index Indian / cleaned Amazon CSV into Qdrant (BGE-M3 dense + sparse).

SERVER_BATCH_SIZE must match TRT engine max batch (2).
Aborts on invalid dense vectors — never silently skips a batch.
"""
import os
import uuid
import numpy as np
...
SERVER_BATCH_SIZE = 2  # must match serve.py / TRT engine max batch
...
def main():
    print("--- Hybrid indexing (cleaned Indian CSV) ---")
