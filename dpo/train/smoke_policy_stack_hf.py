...
def run_skeleton(
    ...
    for i, user_msg in enumerate(skeleton["user_turns"]):
        ...
        text = generate_turn(model, tokenizer, messages)
...
def main() -> None:
    ...
    print(f"  policy stack: {POLICY_ADAPTER_STACK}")

    conversations = []
    for sk in skeletons:
        print(f"\n  Skeleton {sk['id']} (type={sk['opening_type']})")
        messages = run_skeleton(model, tokenizer, sk)
    ...
    payload = {
        ...
        "policy": "hf_bnb_policy_stack",
        "description": "BnB 8-bit base + set_adapter([default, dpo]) — matches DPO training forward",
        "base_model": str(MODEL_ID_BF16),
        "sft_adapter": str(SFT_ADAPTER),
        "dpo_adapter": str(dpo_path),
        "active_adapters": POLICY_ADAPTER_STACK,
